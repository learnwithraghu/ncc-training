...
- `port-forward` — tunnel Pod port 80 to your laptop on 8080.
...
4. In one terminal: `kubectl port-forward pod/orbital-relay 8080:80`.
   In another: `curl -s http://localhost:8080 | grep -o "Ground link v1"`.
   Open `http://localhost:8080` — dark dashboard, **VERSION 1.0**.

## Task

`curl http://localhost:8080` shows **Ground link v1** from a Pod in the
